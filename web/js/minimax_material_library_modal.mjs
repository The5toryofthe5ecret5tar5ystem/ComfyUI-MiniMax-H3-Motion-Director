...
export function mountMaterialLibrary(editor, node = null) {
    ...
    const renderCategories = () => {
        ...
        for (const category of categoriesForType(state.activeType)) appendSubtab(category, materialCategoryLabel(state.activeType, category));
    ...
    const createCard = (item) => {
        ...
        const sub = document.createElement("div"); sub.className = "mmx-ml-card-sub"; sub.textContent = `${materialCategoryLabel(item.type, item.category)} · ${mlT(item.type)}`;
    ...
    const openItemEditor = (item = null) => {
        ...
        for (const value of categoriesForType(type)) { const o = document.createElement("option"); o.value = value; o.textContent = materialCategoryLabel(type, value); cat.appendChild(o); }
