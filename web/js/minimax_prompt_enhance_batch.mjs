...
let activeRun = null;
// The LLM settings the run used, so a single row can be re-run exactly as the batch
// was (and so a regeneration does not re-read a panel that may have changed since).
let activeCfg = null;
...
function buildOverlay(editor, pe, rows, cfg) {
    ...
    /**
     * Re-run one row through the same call the batch used.
     *
     * A list of ten story prompts is not accepted or rejected as a whole: one segment
     * usually misses, and re-running the whole batch to fix it costs ten model passes
     * and loses the nine that were fine. The row keeps its original text, so a
     * regeneration is always from the same source.
     */
    async function regenerate(row, index) {
        const button = regenButtons[index];
        const preview = previews[index];
        const label = button.textContent;
        button.disabled = true;
        button.textContent = t("pe.regenerating");
        try {
            const result = await pe.callEnhanceApi(row.original, row.taskKey, row.block, {
                ...activeCfg,
                skipVision: true,
            });
            if (result?.ok && result.text) {
                row.enhanced = result.text;
                if (preview) preview.textContent = truncate(result.text, 700);
            } else {
                pe.setStatus(t("pe.regenerateFailed"), "error");
            }
        } catch (error) {
            console.warn(`[MiniMax H3 PE] regenerate segment ${row.segmentIndex + 1} failed:`, error);
            pe.setStatus(t("pe.regenerateFailed"), "error");
        } finally {
    ...
    for (const row of rows) {
        ...
        const preview = el("div", { fontSize: "11px", lineHeight: "1.45", color: "#c7d2fe" }, [truncate(row.enhanced, 700)]);
        previews.push(preview);
        const body = el("div", { display: "flex", flexDirection: "column", gap: "4px", minWidth: "0", flex: "1 1 auto" });
        body.append(
        ...
        const regen = button(t("pe.regenerate"), { padding: "4px 8px", fontSize: "10px", flex: "0 0 auto" },
            (event) => { event.preventDefault(); event.stopPropagation(); void regenerate(row, regenButtons.indexOf(regen)); });
        regenButtons.push(regen);
    ...
    const regenAllBtn = button(t("pe.regenerateAll"), {}, async (event) => {
        event.preventDefault();
        // Rows already regenerated by hand are left alone: this is for "the run came
        // out poorly", not for throwing away a row that was just fixed.
        regenAllBtn.disabled = true;
        try {
            for (let index = 0; index < rows.length; index += 1) {
                if (!checkboxes[index].checked) continue;
                await regenerate(rows[index], index);
            }
        } finally {
    ...
    footer.append(cancelBtn, regenAllBtn, applyBtn);
...
export async function runPromptEnhanceBatch(editor, pe) {
    ...
    const { overlay } = buildOverlay(editor, pe, rows, activeCfg);
