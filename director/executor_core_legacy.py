...
def _auto_mask_for_window(body_raw, replace_spec, lead_frames=0):
    ...
    Delegates the anchor/relaxed retry policy to
    :func:`sam3_auto.run_window_auto_mask` so the in-run path and the per-window
    "Test mask" route behave identically.
    """
    ...
    try:
        ...
        if result.get("mask") is not None:
            return result["mask"], ""
        return None, result.get("reason") or "auto SAM3 mask unavailable"
    except Exception as exc:
        log.warning("Segment auto SAM3 mask failed: %s", exc)
        return None, f"auto SAM3 mask failed: {exc}"


from .mask_preview import compose_mask_check_jpeg as _compose_mask_check_jpeg


def _maybe_send_mask_check(node_id, timeline_slot, visible, mask_vis, motion_ref, lead):
    """Push the mask sanity-check montage to the Live Preview page (best-effort)."""
    try:
        from .progress import report_director_mask_check

        jpeg = _compose_mask_check_jpeg(visible, mask_vis, motion_ref, int(lead or 0))
        if jpeg is None:
            return
        report_director_mask_check(
            node_id,
            segment_index=int(timeline_slot),
            image_b64=jpeg,
            label=f"Segment {int(timeline_slot) + 1} mask check - window start frame",
        )
    except Exception as exc:
        log.debug("Mask-check preview skipped: %s", exc)
    ...
    def _run_one_segment(seg, *, progress_index: int) -> tuple[torch.Tensor, dict[str, Any] | None]:
        ...
        if (
            ...
            if replace_state is None:
                ...
                _maybe_send_mask_check(
                    node_id,
                    timeline_slot,
                    visible_clip_frames,
                    replace_state['mask_vis'],
                    reference_clip_frames,
                    replace_lead,
                )
        report_director_progress(
            node_id, segment_index=progress_index, segment_total=seg_total,
